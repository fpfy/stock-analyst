"""
纯Python演示版本 - 不依赖任何外部库，使用模拟数据展示系统功能
"""
...
class DemoSystem:
    ...
    def _init_demo_data(self):
        ...
        cursor.executemany("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [
            ('000001', '上证指数', '2026-06-11', 3260.0, 3285.0, 3255.0, 3278.0, 450000000000, 5200000000000, 0.55, 3265.0, 3258.0, 3245.0, 3200.0),
            ('000001', '上证指数', '2026-06-10', 3240.0, 3270.0, 3235.0, 3260.0, 430000000000, 4900000000000, 0.62, 3260.0, 3252.0, 3240.0, 3195.0),
            ('399001', '深证成指', '2026-06-11', 10800.0, 10920.0, 10780.0, 10895.0, 520000000000, 6200000000000, 0.88, 10820.0, 10785.0, 10720.0, 10550.0),
        ])

        # 插入股票基本信息
        cursor.executemany("""
            INSERT OR REPLACE INTO stock_basic (ts_code, symbol, name, industry, is_st)
            VALUES (?, ?, ?, ?, ?)
        """, [
            ('300750', '300750', '宁德时代', '电力设备', 0),
            ('002594', '002594', '比亚迪', '汽车', 0),
            ('688981', '688981', '中芯国际', '电子', 0),
            ('600036', '600036', '招商银行', '银行', 0),
            ('601088', '601088', '中国神华', '煤炭', 0),
            ('600030', '600030', '中信证券', '非银金融', 0),
        ])
        ...
        """, [
            # 成长股估值
            ('300750', '2026-06-11', 210.5, 23.5, 22.8, 4.65, 5.2, 5.1, 0.8, 0.9, 9250.0, 8950.0),
            ('002594', '2026-06-11', 285.3, 63.1, 58.5, 10.08, 3.2, 3.1, 0.3, 0.35, 8350.0, 7850.0),
            ('688981', '2026-06-11', 52.8, 42.2, 38.5, 6.21, 6.5, 6.2, 0.5, 0.6, 1050.0, 950.0),
            # 价值股估值
            ('600036', '2026-06-11', 36.8, 9.56, 9.2, 1.13, 4.5, 4.3, 4.2, 4.5, 9250.0, 8750.0),
            ('601088', '2026-06-11', 34.5, 16.0, 15.8, 1.82, 1.8, 1.75, 6.2, 6.5, 6850.0, 5950.0),
            ('600030', '2026-06-11', 22.3, 14.1, 13.8, 1.47, 3.8, 3.6, 2.8, 3.0, 3250.0, 3050.0),
        ])
    ...
    def analyze_macro(self):
        ...
        if len(pmi_results) >= 2:
            ...
            current_pmi = 50.8
            logger.info(f"PMI: {current_pmi:.1f} - 位于荣枯线之上，制造业保持扩张")
        ...
        if cpi_result:
            ...
            logger.info(f"CPI: {cpi_value:.1f}% - 存在通缩压力")
        else:
            cpi_value = 0.3
            logger.info(f"CPI: {cpi_value:.1f}% - 存在通缩压力")
